...
// H-3: optional `timedOut`/`onRetry` — when auth init stalls past the timeout, show a
// clear recovery path under the spinner instead of leaving it spinning forever with no
// way out but a hard refresh. Omitting both props preserves the original appearance
// exactly (every other BootSplash call site is unaffected).
export default function BootSplash({ label = 'Loading your workshop…', timedOut = false, onRetry = null }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(120% 120% at 50% 0%, #141110 0%, #0a0a0a 60%)', fontFamily: 'Montserrat, system-ui, sans-serif' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
        ...
        {timedOut && (
          <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, maxWidth: 260, textAlign: 'center' }}>
            ...
            {onRetry && (
              <button
                type="button"
                onClick={onRetry}
                style={{ fontSize: 12, fontWeight: 700, color: '#0a0a0a', background: 'linear-gradient(135deg,#e8c84a,#aa801e)', border: 'none', borderRadius: 10, padding: '9px 18px', cursor: 'pointer' }}
              >
                Retry
              </button>
            )}
