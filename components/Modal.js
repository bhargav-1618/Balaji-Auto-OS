...
function lockBody() {
  ...
  __tokens.add(token);
  if (__c === 0) {
    // The document no longer scrolls (see lib/appScroll.js): the content area does.
    // So the lock freezes THAT container instead of pinning <body> with position:fixed —
    // pinning the body would now fight the viewport-height shell and shift the layout.
    const sc = getAppScroller();
    if (sc) {
      __y = sc.scrollTop;
      sc.style.overflow = 'hidden';
    } else {
      // No app shell on screen (e.g. the login page, or before the shell mounts).
      // Fall back to the classic body pin so the background still can't scroll.
...
function __releaseBody() {
  ...
  // Clear any legacy body pin (older builds used position:fixed on <body>).
  b.style.position = '';
  b.style.top = '';
  b.style.left = '';
  b.style.right = '';
  ...
  const sc = getAppScroller();
  if (sc) { sc.style.overflow = ''; sc.scrollTop = __y; }
  else if (typeof window !== 'undefined') window.scrollTo(0, __y);
}
...
export default function Modal({
  ...
  // H-6: Escape closes the modal. Centralized here (not per call site) so every dialog
  // built on Modal gets it uniformly — mirrors the Escape handling DropdownPanel/
  // ConfirmDialog already do themselves; the global focus trap (lib/focusTrap.js)
  // deliberately does NOT handle Escape for exactly this reason ("most overlays already
  // bind Esc themselves"). One listener per mounted-and-open instance, removed on
  // unmount/close — cannot duplicate or leak.
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Issue 1 (Add Vehicle popup architecture review) — every dropdown rendered
            anywhere inside this modal (MiniSelect/TreeSelect/SearchSelect) now clamps
            its available height to THIS panel's own rect instead of the full browser
            window, automatically, with no per-field prop needed. See
            components/common/DropdownPanel.jsx's ModalBoundaryContext. */}
        <ModalBoundaryContext.Provider value={panelRef}>
          {dbg && (
