...
const PI_CHANNEL = 'ph7b:pi';
const mintPi = () => PAGE_INSTANCE_PREFIX + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 10);
...
const piSelfNonce = Math.random().toString(36).slice(2);  // ignore our own channel echoes

// PH29-01 — `window.name` alone cannot be trusted as an "am I a fresh context?"
// signal. The Phase 7b fix assumed a duplicated tab always starts with an EMPTY
// `window.name`; that was only ever verified for a plain new tab and a same-tab
// reload, never for the actual "Duplicate tab" gesture. Chromium serialises the
// main frame's name into the tab's navigation PageState, which "Duplicate tab"
// and session restore copy — so a duplicate CAN inherit the origin tab's
// `window.name` alongside its cloned `sessionStorage`, and the page-instance tag
// then matches on both sides (PH7-01 re-opens: a genuinely different operation in
// the duplicate is swallowed as a retry of the original's in-flight one).
//
// This cross-checks against a LIVE sibling over a BroadcastChannel. If any other
// OPEN context is already using this exact page-instance id, BOTH sides re-mint a
// genuinely fresh one — so a sessionStorage entry inherited by cloning is never
// reused for a new intent (readOrCreateOpId / peekOpId already reject an entry
// whose `pi` tag no longer matches). A same-tab RELOAD has NO live sibling, so
// nothing re-mints and the Phase 5b/6b refresh-safety guarantee is untouched.
// No-ops where BroadcastChannel is unavailable (falls back to the window.name
// check alone — no worse than before this).
function startPiCollisionWatch() {
  if (typeof BroadcastChannel === 'undefined') return;
  let bc;
  try { bc = new BroadcastChannel(PI_CHANNEL); } catch { return; }
  const post = (pi) => { try { bc.postMessage({ pi, nonce: piSelfNonce }); } catch { /* channel closed */ } };
  bc.onmessage = (e) => {
    const m = e && e.data;
    if (!m || m.nonce === piSelfNonce || m.pi !== cachedPageInstanceId) return;
    // Another LIVE context holds our exact id — it can only be a clone (or a
    // stale-tab resume racing the original). Echo the colliding id first (so the
    // sibling also learns its id is contested and re-mints too — the original may
    // have announced before we loaded and we'd otherwise never hear it), THEN
    // re-mint and announce the fresh one. Both sides converge on distinct ids and
    // neither trusts an opId inherited through the clone. Messages for an id we no
    // longer hold are ignored, so this cannot loop.
    const contested = cachedPageInstanceId;
    post(contested);
    cachedPageInstanceId = mintPi();
    try { window.name = cachedPageInstanceId; } catch { /* ignore */ }
    post(cachedPageInstanceId);
  };
  try { window.addEventListener('pagehide', () => { try { bc.close(); } catch {} }, { once: true }); } catch { /* ignore */ }
  post(cachedPageInstanceId);
}
...
function getPageInstanceId() {
  ...
  try {
    let pi = (typeof window.name === 'string' && window.name.startsWith(PAGE_INSTANCE_PREFIX))
      ? window.name
      : null;
    if (!pi) {
      pi = mintPi();
      try { window.name = pi; } catch { /* window.name write blocked — fall through, still return fresh for THIS read */ }
    }
    cachedPageInstanceId = pi;
    startPiCollisionWatch();   // PH29-01 — re-mints if a live sibling shares this id
...
// PH29-01 — derive the page-instance id NOW, at first import (app boot), not
// lazily on the first durable-op modal. This starts the BroadcastChannel
// collision watch as early as possible, so a duplicated tab's id is re-minted
// long before any operation modal can read an inherited opId out of the cloned
// sessionStorage. Guarded for SSR and wrapped so a throw here can never break
// module evaluation.
if (typeof window !== 'undefined') {
  try { getPageInstanceId(); } catch { /* ignore */ }
}
