...
function genRollups(sales) {
  ...
  for (const s of sales) {
    ...
    // BUG-LIVE-005: `cost` was never accumulated here, so the Analytics "Monthly Profit
    // Trend" (which reads rollups) showed COST ₹0 / MARGIN 100% while every other panel
    // on the page — built from the sales ledger, which does carry cost — showed real
    // margins. Production rollups (InventoryDashboard writeRollups) already store `cost`;
    // this brings the demo seed to the same shape. Each sale record carries `s.cost`.
    if (!map[key]) map[key] = { id: key, month: key, revenue: 0, cost: 0, profit: 0, count: 0 };
    map[key].revenue += s.totalPrice;
    map[key].cost += (s.cost || 0);
    map[key].profit += s.profit;
    map[key].count += s.quantity;
  }
  return Object.values(map).sort((a, b) => (a.month < b.month ? 1 : -1));
}

// BUG-LIVE-006: the old version fabricated 520 entries as
//   { action: 'Created part', detail: 'Demo activity', user: 'demo@guest' }
// — a shape that matches neither the real audit writers (writeAudit / pushAudit in
// InventoryDashboard.js, which store a machine-key `action`, an entity `name`, and
// `performedByEmail`) nor what AuditRow renders. Every row therefore showed
// "Created part — unknown" with no part and no user. This regenerates the demo audit
// trail from the actual seeded parts/suppliers using the real action keys, so the
// Activity / Audit Log panel is meaningful in demo mode.
function genAuditLog(parts, suppliers) {
  const ACTOR = 'demo@balajiautoos.com';
  const REASONS = ['Damage', 'Lost Item', 'Personal Use', 'Correction'];
  const at = (day) => ts(new Date(Date.now() - day * 86400000 - between(0, 82800) * 1000));
  const out = [];
  for (let i = 0; i < 260; i++) {
    const createdAt = at(between(0, 30));
    const roll = rnd();
    if (roll < 0.30 && parts.length) {
      const p = pick(parts);
      const before = between(0, 40);
      const delta = between(1, 8) * (rnd() < 0.4 ? -1 : 1);
      out.push({ id: `demo-au-${i}`, action: 'stock_adjustment', partId: p.id, name: p.name, performedByEmail: ACTOR, createdAt,
        details: { reason: pick(REASONS), qty: delta, stockBefore: before, stockAfter: Math.max(0, before + delta), notes: '' } });
    } else if (roll < 0.48 && parts.length) {
      const p = pick(parts);
      const from = Math.round(p.sellingPrice || between(200, 4000));
      const to = Math.round(from * (0.9 + rnd() * 0.25));
      out.push({ id: `demo-au-${i}`, action: 'price_change', partId: p.id, name: p.name, performedByEmail: ACTOR, createdAt,
        details: { 'Selling price': { from, to } } });
    } else if (roll < 0.64 && parts.length) {
      const p = pick(parts);
      out.push({ id: `demo-au-${i}`, action: 'create_part', partId: p.id, name: p.name, performedByEmail: ACTOR, createdAt, details: '' });
    } else if (roll < 0.78 && parts.length) {
      const p = pick(parts);
      const qty = between(1, 5);
      const unitPrice = Math.round(p.sellingPrice || between(200, 3000));
      out.push({ id: `demo-au-${i}`, action: 'sell_part', partId: p.id, name: p.name, performedByEmail: ACTOR, createdAt,
        details: { qty, unitPrice, revenue: qty * unitPrice } });
    } else if (roll < 0.90 && (suppliers || []).length) {
      const s = pick(suppliers);
      out.push({ id: `demo-au-${i}`, action: rnd() < 0.5 ? 'create_supplier' : 'update_supplier', supplierId: s.id, name: s.name, performedByEmail: ACTOR, createdAt, details: '' });
    } else if (parts.length) {
      const p = pick(parts);
      out.push({ id: `demo-au-${i}`, action: rnd() < 0.5 ? 'archive_part' : 'restore_part', partId: p.id, name: p.name, performedByEmail: ACTOR, createdAt, details: '' });
    }
  }
  return out.sort((a, b) => b.createdAt.seconds - a.createdAt.seconds);
}
...
export function getDemoData() {
  ...
  // PH23-D3 — `salesCount` is a part's lifetime units-sold counter. genParts() seeds a
  // random value, but the app maintains it from real sales (Quick Sell + invoice
  // realization). Reconcile it to the DERIVED ledger so demo Dead Stock / Dead Capital
  // / Fast Mover analytics agree with the sales the demo actually shows — otherwise a
  // part with 14 units / ₹39k of ledger sales but a random salesCount of 0 tops the
  // "never-sold" Dead Stock list.
  {
    const soldUnits = {};
    sales.forEach((s) => { if (s.partId) soldUnits[s.partId] = (soldUnits[s.partId] || 0) + (Number(s.qty) || 0); });
    parts.forEach((p) => { p.salesCount = Math.max(0, soldUnits[p.id] || 0); });
  }
  ...
  const auditLog = genAuditLog(parts, suppliers);
