 ...
 *
 * `serverDoc` (ID-7) — the authoritative document revState was actually called
 * with (the current server/localStorage row, or null when deleted) — is
 * attached as `.current` on a 'stale' rejection. Both guardedSet (Firestore)
 * and saveGuarded (demo/localStorage) already read this document to DETECT the
 * conflict in the first place; passing it through here costs nothing extra and
 * lets a caller refresh its own stale list entry with real, current data
 * instead of leaving it stale until a manual reload — without weakening the
 * rejection itself, which still throws exactly as before.
 */
export function conflictError(state, label, serverDoc) {
  ...
  if (state.conflict === 'stale') {
    const err = new ConcurrencyError(CONC_STALE, `${label || 'This record'} was changed by another user after you opened it.`);
    err.current = serverDoc || null;
    return err;
  }
  return null;
}
