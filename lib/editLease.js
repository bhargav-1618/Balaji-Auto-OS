...
import {
  db, doc, setDoc, deleteDoc, onSnapshot, runTransaction, serverTimestamp, Timestamp,
} from './firebase';
...
/** Extend my lease (heartbeat). Best-effort; a rejection means I no longer own it. */
export async function renewLease(collectionName, docId, { uid, email, sessionId }) {
  await setDoc(leaseRef(collectionName, docId), {
    ownerUid: uid,
    ownerEmail: email || '',
    sessionId,
    acquiredAt: serverTimestamp(),
...
/** Release my lease. Best-effort — never blocks the UI, expiry is the backstop. */
export async function releaseLease(collectionName, docId) {
  try {
    await deleteDoc(leaseRef(collectionName, docId));
  } catch (e) {
