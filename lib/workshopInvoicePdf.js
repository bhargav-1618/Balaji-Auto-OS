...
import { PDF_PAGE, PDF_GOLD, PDF_RULE, PDF_TEXT, drawPdfHeader, truncW } from './pdfTheme';
import { QR_PT } from './pdfQr';
import { num, asArray } from './format';
...
// Truncate-with-ellipsis (`truncW`) moved to lib/pdfTheme.js in Phase 22 (PH22-04) —
// it was duplicated there verbatim and, worse, diverged into an ellipsis-less
// `.slice(0, 52)` in BillingModule's customer-copy invoice PDF. One copy now.
...
function renderJobCardCard(doc, ctx, { iv, jc }) {
  ...
  const delivered = asArray(jc.statusLog).find((s) => s.status === 'Delivered');
  const shortFields = [
  ...
  const longFields = [
    ['Complaint', asArray(jc.complaints).filter(Boolean).join('; ')],
    ['Diagnosis', asArray(jc.diagnosis).filter(Boolean).join('; ')],
...
function renderFinancialSummaryCard(doc, ctx, { iv, t, money }) {
  ...
  const payments = asArray(iv.payments); // PH22-02 — a wrong-type `payments` must not crash PDF generation
