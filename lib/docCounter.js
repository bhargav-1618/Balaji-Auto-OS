...
import { withTimeout, TX_TIMEOUT_MS } from './txTimeout';
...
export async function allocateNumber(sequence, seedFrom = 1) {
  ...
  // Phase 6b (PH6-03) — bound the UI wait. Safe by the SAME self-healing property
  // documented above: if this call times out on the UI side but the transaction
  // later commits anyway, the counter only ever moves forward (Math.max), so a
  // caller that gives up and tries again on a fresh attempt gets a NEW, higher
  // number — never the same one twice, never a duplicate. Worst case is exactly
  // the pre-existing "gap" outcome this file already documents as legal.
  return withTimeout(runTransaction(db, async (tx) => {
    ...
    return allocated;
  }), TX_TIMEOUT_MS, 'Allocating the document number');
}
