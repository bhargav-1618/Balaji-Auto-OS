...
//   - The client generates ONE opId per logical user intent — in a `useRef` or a
//     `useState` initialiser on the modal/form that owns the action — and reuses it
//     for every retry of that same intent.
...
//   - It does NOT survive a full browser refresh (a `useRef` is in memory only). On
//     reload the user starts a new intent; if the prior operation had committed it
//     is visible in the record. This residual limitation is documented in
