...
// Real workshop identity, out of tracked source (privacy cleanup) — NEXT_PUBLIC_*
// is required, not a shortcut: this module is imported directly (not dynamically)
// by client components, so it ships in the browser bundle regardless, and only
// NEXT_PUBLIC_-prefixed vars get inlined into that bundle at build time (an
// unprefixed process.env.X would just read `undefined` in the browser and always
// fall back to the placeholder). These are configuration, not secrets — a
// workshop's name/phone/address/GST/email/website are printed on every PDF handed
// to a customer, so browser visibility is nothing new; the only thing moving is
// where the *default* value lives (env var vs. hardcoded in Git). Unset in a given
// environment, these fall back to generic placeholders rather than crashing.
export const SHOP = {
  name: process.env.NEXT_PUBLIC_SHOP_NAME || 'Your Workshop Name',
  tag: process.env.NEXT_PUBLIC_SHOP_TAGLINE || 'Automotive Service & Diagnostics',
  phones: process.env.NEXT_PUBLIC_SHOP_PHONES || '00000 00000',
  address: process.env.NEXT_PUBLIC_SHOP_ADDRESS || 'Your workshop address, city, state, PIN',
  gst: process.env.NEXT_PUBLIC_SHOP_GST || '22AAAAA0000A1Z5',
  email: process.env.NEXT_PUBLIC_SHOP_EMAIL || 'contact@example.com',
  website: process.env.NEXT_PUBLIC_SHOP_WEBSITE || 'example.com',
};
 ...
 *  O(chars) measure-one-char-at-a-time loop on large reports. Measures against the
 *  font/size/weight CURRENTLY set on `doc`, so a value uses the column's real
 *  available space rather than a guessed character count.
 *
 *  PHASE 22 (PH22-04) — promoted from module-private to exported. There were THREE
 *  copies of this exact routine: this one (report tables), an identical `truncW` in
 *  lib/workshopInvoicePdf.js, and — the one that mattered — a flat `.slice(0, 52)`
 *  with NO ellipsis for line descriptions in BillingModule's customer-copy invoice
 *  PDF. So the same invoice's line "Front brake pad set (LEFT) — ceramic OEM …"
 *  printed in full (2-line wrap) on the workshop copy but was silently chopped to
 *  "Front brake pad set (LEFT) — cera" on the customer copy, with nothing to signal
 *  the text had been cut. One implementation now. */
export function truncW(doc, text, maxWidth) {
  const s = String(text ?? '');
...
/** Report-cell flavour of {@link truncW}: routes the value through `cellText` first
 *  so a raw Date object (see below) is never stringified as-is. */
function fitText(doc, text, maxW) {
  return truncW(doc, cellText(text), maxW);
}
