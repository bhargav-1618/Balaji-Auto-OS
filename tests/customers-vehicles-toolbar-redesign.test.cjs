...
// --- Customers ---
{
  ...
  ok('Customers: Row 2 still has the Customer Type filter, Status filter, Export, and New Customer — nothing dropped',
    /<MiniSelect value=\{typeF\}/.test(block) && /<MiniSelect value=\{statusF\}/.test(block) && /onClick=\{exportCSV\}/.test(block) && /New Customer/.test(block));
...
// --- Vehicles ---
{
  ...
  ok('Vehicles: Row 2 still has Make/Fuel/Status filters, the Sort dropdown, Export, and Add Vehicle — nothing dropped',
    /<MiniSelect value=\{makeF\}/.test(block) && /<MiniSelect value=\{fuelF\}/.test(block) && /<MiniSelect value=\{statusF\}/.test(block) && /<MiniSelect value=\{sortBy\}/.test(block) && /onClick=\{exportCSV\}/.test(block) && /Add Vehicle/.test(block));
}
