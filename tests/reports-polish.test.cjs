...
// Refactor Phase 8 — ReportTable + ReportsView moved verbatim to this file.
...
ok('inRange handles invalid dates', /if \(Number\.isNaN\(d\.getTime\(\)\)\) return false; return d >= startOfRange/.test(rv));
ok('inRangeMs added for ms timestamps', /const inRangeMs = \(ms\) =>/.test(rv));
ok('sales rows filtered by range', /invoices\.filter\(\(iv\) => !iv\.isEstimate && inRange\(iv\.date\)\)/.test(rv));
ok('billing rows filtered by range', /invoices\.filter\(\(iv\) => inRange\(iv\.date\)\)\.map/.test(rv));
ok('parts/service/labour/outside sales filtered by range', /cats\.includes\(catOfSale\(s\)\) && inRangeMs\(saleMs\(s\)\)/.test(rv));
ok('jobcard rows filtered by range', /jobCards\.filter\(\(j\) => inRange\(j\.date \|\| j\.dateIn\)\)/.test(rv));
ok('gst rows filtered by range', /invStatus\(iv\) !== 'Cancelled' && inRange\(iv\.date\)/.test(rv));
...
ok('audit rows filtered by range', /auditLog\.filter\(\(a\) => inRangeMs\(auditAtMs\(a\)\)\)/.test(rv) && /const auditAtMs = \(a\) => \{/.test(rv));
ok('snapshot tabs identified (range N/A)', /const DATE_FILTERABLE = new Set\(\[/.test(rv));
ok('range disabled + explained on snapshot tabs', /disabled=\{!DATE_FILTERABLE\.has\(tab\)\}/.test(rv) && /date range does not apply/.test(rv));
...
ok('Reports imports and uses the shared PageHeader', /import PageHeader from '\.\.\/\.\.\/common\/PageHeader'/.test(rv) && /<PageHeader title="Reports" icon=\{FileText\}/.test(rv));
ok('Reports header shows active range in subtitle', /subtitle=\{DATE_FILTERABLE\.has\(tab\)/.test(rv) && /current snapshot/.test(rv));
ok('range select has focus ring + aria', /aria-label="Report date range"/.test(rv) && /disabled:cursor-not-allowed"/.test(rv) && /focus:border-\[#d4af37\]\/60 disabled:opacity-40/.test(rv));

// Part 2 — numeric alignment
ok('numeric columns detected', /const numericCols = useMemo/.test(rv) && /NUM_HEAD = /.test(rv));
ok('numeric headers right-aligned', /\$\{numericCols\[i\] \? 'text-right' : 'text-left'\}/.test(rv));
ok('numeric cells right-aligned + tabular', /\$\{numericCols\[j\] \? 'text-right tabular-nums' : ''\}/.test(rv));

// search clear
ok('search has clear button', /aria-label="Clear search"[\s\S]{0,80}setQ\(''\)/.test(rv) || /onClick=\{\(\) => setQ\(''\)\} aria-label="Clear search"/.test(rv));
