...
ok('downloadPDF is a thin single-invoice wrapper: creates the doc, calls drawInvoiceDocument, saves/prints — same output as before (ID-6 added a popup-safe print window open ahead of the async work; see billing-print-popup-safe.test.cjs)',
...
const combinedBlock = bill.slice(combinedStart, combinedStart + 1900);
...
ok('exactly ONE save/print call for the whole batch (doc.save or finishPrintWindow, which itself calls doc.autoPrint()+window.open exactly once — ID-6), not one per invoice',
  (combinedBlock.match(/doc\.save\(|finishPrintWindow\(printWin, doc\)/g) || []).length === 2); // one save branch + one print-finish branch, mutually exclusive at runtime
