...
- **Refund/Return no longer double-restores stock, and a concurrent
  ...
  Former residual — **now fixed (PHASE 22, PH22-03):** `deriveStatus`'s
  "nothing paid yet" label is `"Unpaid"`; `invStatus` / `billingService`'s
  equivalent branch used to be `"Pending"`, so the Reports→Billing export
  (driven by `invStatus`) could show "Pending" for the same invoice the
  Billing list export (driven by `deriveStatus`) called "Unpaid".
  `INVOICE_STATUS.PENDING` is now the string `"Unpaid"` and `invStatus`'s
  last branch returns `"Unpaid"` — every screen and every export agree.
...
- **Every export matches the authoritative record it was generated from** (PHASE 22 —
  ...
  `docs/testing/PHASE_22_PDF_EXPORT_INTEGRITY_REPORT.md`). The invoice PDF, invoice
  XLSX, QR payload and `/verify` page were checked — automated and live in demo mode
  against real generated bytes — with an independent hand-calculation that is never
  derived from the output: grand / GST / discount / round-off / paid / balance /
  status / record identity / record count all agree. No export omits, duplicates or
  misattributes a record. **PH22-01 (MEDIUM, fixed):** `billingService.invoiceTotals`
  (the third money-path copy, behind the Vehicle Report "Revenue" export via
  `lib/vehicleStats.revenueOf`) was a simplified model — it ignored the invoice-level
  discount, `gstMode` (exempt / IGST) and the per-line-GST-absent → `gstPct` fallback,
  so a discounted or GST-exempt invoice's contribution to exported vehicle revenue was
  overstated, and `invoiceStatus`/`isRealized` could disagree with the Billing
  screen's `deriveStatus` on those invoices. Body rewritten to mirror
  `BillingModule.totalsOf`'s full model; `financial-integrity.test.cjs` now checks all
  three copies against the oracle. **PH22-02 (MEDIUM, fixed):** the invoice-PDF
  generators (`drawInvoiceDocument`, `lib/workshopInvoicePdf.js`) still used
  `iv.lines || []` / `iv.payments || []` / `jc.statusLog || []` — the PH21-D1
  wrong-type-array class, in the one path PH21-D1's sweep did not reach; a forged
  non-array field crashed PDF generation. `asArray` guards added. **PH22-03 (LOW,
  fixed):** the "Pending" vs "Unpaid" status-vocabulary split (see the Phase 11 entry
  above). **PH22-04 (LOW, fixed):** the customer-copy invoice PDF and the Purchase
  Order PDF cut line/item descriptions at `.slice(0, 52)` with **no ellipsis**, while
  the workshop copy of the same invoice wraps the full text — silent truncation, and
  two near-identical part names could collapse to the same string. The three
  copies of "truncate to a width with an ellipsis" in the PDF layer
  (`pdfTheme.fitText`, `workshopInvoicePdf.truncW`, the bare `.slice`) are now one
  exported `pdfTheme.truncW`. No `firestore.rules` change.
  Residual, by design (not defects):
  - The invoice **list XLSX** rounds every money column to whole rupees — it is a
    management summary; the Grand Total is exact and the columns reconcile to it, and
    the exact-paisa tax figure is on the invoice PDF and in the GST / GSTR-1 report.
  - Non-Latin scripts (CJK / Devanagari / Arabic) and `₹` render as mojibake in any
    jsPDF output — the built-in Helvetica is WinAnsi-only. The money and
    record-identity paths are unaffected (numbers are ASCII; `₹` is already printed as
    `"Rs. "`), and `/verify` (a real web font) renders every script correctly.
  - `/verify` displays the QR's URL parameters verbatim with **no Firestore lookup** —
    intentional, so a customer who is logged out or offline can still read it; the
    page disclaims "if this doesn't match your printed copy, contact the workshop". A
